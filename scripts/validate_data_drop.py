...
CANONICAL_PDL1_BUCKETS = {"lt1", "1to49", "ge50", "any", "unspecified"}
...
CANONICAL_INTERVENTION_TAG_RE = re.compile(r"^[a-z0-9][a-z0-9\-_]*$")
...
CANONICAL_DISEASE_SETTING_ALIASES = {"early_stage": "early"}
...
def _validate_esmo_external_v2(records: list[dict[str, Any]], report: ValidationReport) -> None:
    seen_ids: set[str] = set()
    for item in records:
        record_id = str(item.get("topicId", "<missing>"))
        missing = _has_missing_fields(item, ESMO_EXTERNAL_V2_REQUIRED_FIELDS)
        if missing:
        ...
        disease_setting = _normalize_token(str(item.get("diseaseSetting", "")))
        histology = _normalize_token(str(item.get("histology", "")))
        line_of_therapy_raw = _normalize_token(str(item.get("lineOfTherapy", "")))
        line_of_therapy = ESMO_LINE_OF_THERAPY_ALIASES.get(line_of_therapy_raw, line_of_therapy_raw)
        stance = item.get("guidelineStance")

        if disease_setting not in CANONICAL_DISEASE_SETTINGS:
            report.add("error", "invalid_disease_setting", f"Unknown diseaseSetting `{item.get('diseaseSetting')}`.", record_id)
        if histology not in CANONICAL_HISTOLOGY:
            report.add("error", "invalid_histology", f"Unknown histology `{item.get('histology')}`.", record_id)
        if line_of_therapy not in CANONICAL_LINE_OF_THERAPY:
            report.add("error", "invalid_line_of_therapy", f"Unknown lineOfTherapy `{item.get('lineOfTherapy')}`.", record_id)
        if stance not in CANONICAL_STANCE:
            report.add("error", "invalid_guideline_stance", f"Unknown guidelineStance `{stance}`.", record_id)
        ...
        biomarker_requirements = item.get("biomarkerRequirements")
        ...
        if not isinstance(biomarker_logic, dict):
            ...
            for key in ("anyPositive", "allNegative"):
                value = biomarker_logic.get(key, [])
                if not isinstance(value, list):
                    report.add("error", "invalid_biomarker_logic", f"`biomarkerLogic.{key}` must be a list.", record_id)
                else:
                    for gene in value:
                        if gene not in CANONICAL_BIOMARKER_KEYS - {"PDL1Bucket"}:
                            report.add("warning", "unknown_biomarker_key", f"Unknown biomarker key `{gene}` in `{key}`.", record_id)
        ...
        semantic_normalization = item.get("semanticNormalization")
        if not isinstance(semantic_normalization, dict):
            ...
            if not isinstance(ontology_tags, dict):
                report.add("error", "invalid_semantic_normalization", "`semanticNormalization.ontologyTags` must be an object.", record_id)
            else:
                for key in ("layer1", "layer2", "layer3"):
                    if not isinstance(ontology_tags.get(key, []), list):
                        report.add("error", "invalid_semantic_normalization", f"`ontologyTags.{key}` must be a list.", record_id)
...
def _validate_pubmed_raw_v2(
    ...
    if evidence_type not in CANONICAL_EVIDENCE_TYPES:
        ...
        if mapped is None:
            ...
            severity = "error" if strict_mvp_pubmed else "warning"
            code = "manual_review_required" if mapped == "unspecified" else "noncanonical_evidence_type_alias"
            message = (
                "evidenceType could not be determined reliably from source metadata and will require manual review."
                if mapped == "unspecified"
                else f"evidenceType `{item.get('evidenceType')}` should be sent as `{mapped}`."
            )
            report.add(severity, code, message, record_id)
    ...
    for field, allowed in (
        ...
        if "," in raw_value:
            ...
            normalized = CANONICAL_DISEASE_SETTING_ALIASES.get(normalized, normalized) if field == "diseaseSetting" else normalized
        if normalized not in allowed:
            report.add("error", f"invalid_{field}", f"Unknown {field} `{raw_value}`.", record_id)
        elif field == "diseaseSetting" and _normalize_token(raw_value) in CANONICAL_DISEASE_SETTING_ALIASES:
            report.add(
                "warning",
                "noncanonical_disease_setting_alias",
                f"diseaseSetting `{raw_value}` should be sent as `{normalized}`.",
                record_id,
            )
...
def _extract_json_records(payload: Any, report: ValidationReport) -> list[dict[str, Any]]:
    if isinstance(payload, list):
        return _validate_json_list(payload, report)
    if isinstance(payload, dict) and isinstance(payload.get("records"), list):
        return _validate_json_list(payload["records"], report)
    report.add("error", "not_a_list", "Expected the file payload to be a JSON array or an object with `records`.")
    return []
...
def _detect_dataset_shape(path: Path) -> tuple[str, list[dict[str, Any]]]:
    ...
    records = _extract_json_records(payload, ValidationReport("unknown", "unknown", str(path)))
    if records and "Topic_ID" in records[0]:
        return "raw", records
    if records and "biomarkerRequirements" in records[0] and "biomarkerLogic" in records[0]:
        return "external_v2", records
...
def validate_dataset(path: Path, dataset_kind: str, *, strict_mvp_pubmed: bool = False) -> ValidationReport:
    ...
    if path.suffix.lower() == ".json":
        records = _extract_json_records(_read_json(path), report)
    ...
    if dataset_kind == "esmo":
        ...
        elif dataset_shape == "external_v2":
            _validate_esmo_external_v2(records, report)
        else:
