import os

from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware

from app.api.routes.catalog import router as catalog_router
from app.api.routes.evals import router as evals_router
from app.api.routes.governance import router as governance_router
from app.api.routes.imports import router as imports_router
from app.api.routes.meta import router as meta_router
from app.api.routes.runs import router as runs_router
from app.config.settings import settings
from app.config.versioning import load_version_manifest
from app.repositories.bootstrap import bootstrap_database
from app.services.runtime_prewarm_service import runtime_prewarm_service

version_manifest = load_version_manifest()

app = FastAPI(
    title=settings.app_name,
    version=str(version_manifest["backendVersion"]),
    description="Deterministic NSCLC evidence triage API with governance and evaluation scaffolding.",
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,
    allow_methods=["*"],
    allow_headers=["*"],
)

app.include_router(runs_router)
app.include_router(catalog_router)
app.include_router(evals_router)
app.include_router(governance_router)
app.include_router(imports_router)
app.include_router(meta_router)


@app.on_event("startup")
def startup_event():
    if not os.getenv("VERCEL"):
        bootstrap_database()
        return

    if os.getenv("VERCEL_ENV") == "production":
        build_key = f"{version_manifest['backendVersion']}:{version_manifest['buildLabel']}"
        runtime_prewarm_service.schedule_post_deploy_prewarm(
            build_key=build_key,
            include_semantic=True,
            include_benchmark=True,
        )


@app.get("/")
def root():
    return {
        "service": settings.app_name,
        "status": "ok",
        "version": version_manifest,
        "endpoints": {
            "health": "/health",
            "docs": "/docs",
            "openapi": "/openapi.json",
        },
    }


@app.get("/health")
def healthcheck():
    return {
        "status": "ok",
        "service": settings.app_name,
        "databaseUrl": settings.database_url,
        "backendVersion": version_manifest["backendVersion"],
        "productVersion": version_manifest["productVersion"],
        "buildLabel": version_manifest["buildLabel"],
    }
