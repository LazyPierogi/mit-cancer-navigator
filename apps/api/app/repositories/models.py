...
class SourceDocumentRecord(Base):
    ...
    histology_original: Mapped[str | None] = mapped_column(String(128), nullable=True)
    histology_normalized: Mapped[str] = mapped_column(String(64), default="unspecified")
    histology_source: Mapped[str] = mapped_column(String(64), default="unknown")
...
class DocumentChunkRecord(Base):
    ...
    source_id: Mapped[str] = mapped_column(String(128), index=True)
    topic_id: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    title: Mapped[str] = mapped_column(Text)
    chunk_text: Mapped[str] = mapped_column(Text)
    dense_vector: Mapped[list] = mapped_column(JSON, default=list)
    sparse_vector: Mapped[dict] = mapped_column(JSON, default=dict)
    metadata_payload: Mapped[dict] = mapped_column(JSON, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class EmbeddingJobRecord(Base):
    ...
    retrieval_mode: Mapped[str] = mapped_column(String(64))
    embedding_model: Mapped[str] = mapped_column(String(128))
    chunking_strategy_version: Mapped[str] = mapped_column(String(128))
...
class ProjectionPointRecord(Base):
    __tablename__ = "projection_points"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    point_id: Mapped[str] = mapped_column(String(128), unique=True, index=True)
    ...
    topic_id: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    title: Mapped[str] = mapped_column(Text)
    histology: Mapped[str] = mapped_column(String(64), default="unspecified")
