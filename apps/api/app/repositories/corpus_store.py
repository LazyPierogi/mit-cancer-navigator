...
class CorpusStore:
    ...
    def merge_guideline_topics(self, *, batch_id: str, topics: Sequence[dict]) -> dict[str, int]:
        with SessionLocal() as session:
            existing_records = session.execute(select(GuidelineTopicRecord)).scalars().all()
            existing_by_topic_id = {record.topic_id: record for record in existing_records}
            added_count = 0
            updated_count = 0

            for topic in topics:
                record = existing_by_topic_id.get(topic["topicId"])
                if record is None:
                    session.add(
                        GuidelineTopicRecord(
                            topic_id=topic["topicId"],
                ...
                record.topic_title = topic["topicTitle"]
                record.topic_applicability = topic["topicApplicability"]
                record.topic_intervention_tags = topic["topicInterventionTags"]
                record.guideline_stance = topic["guidelineStance"]
                record.stance_notes = topic.get("stanceNotes")
                record.prerequisites = topic.get("prerequisites", [])
                updated_count += 1
    ...
    def append_evidence_studies(self, *, batch_id: str, evidence_records: Sequence[dict]) -> dict[str, int]:
        with SessionLocal() as session:
            incoming_ids = [record["evidenceId"] for record in evidence_records]
            existing_records = session.execute(
                select(EvidenceStudyRecord).where(EvidenceStudyRecord.evidence_id.in_(incoming_ids))
            ).scalars().all()
            existing_by_id = {record.evidence_id: record for record in existing_records}

            added_count = 0
            updated_count = 0
            for payload in evidence_records:
                existing = existing_by_id.get(payload["evidenceId"])
                if existing is None:
                    session.add(
                        EvidenceStudyRecord(
                            ...
                            import_batch_id=batch_id,
                            title=payload["title"],
                            publication_year=payload.get("publicationYear"),
                            evidence_type=payload["evidenceType"],
                            source_category=payload.get("sourceCategory"),
                ...
                existing.publication_year = payload.get("publicationYear")
                existing.evidence_type = payload["evidenceType"]
                existing.source_category = payload.get("sourceCategory")
        ...
        return {
            "processedCount": len(evidence_records),
            "addedCount": added_count,
            "updatedCount": updated_count,
        }
    ...
    def get_guideline_topics(self) -> list[dict]:
        with SessionLocal() as session:
            records = session.execute(select(GuidelineTopicRecord)).scalars().all()
            return [
                {
    ...
    def get_evidence_studies(self) -> list[dict]:
        with SessionLocal() as session:
            records = session.execute(select(EvidenceStudyRecord)).scalars().all()
    ...
    def get_latest_import_batch(self, *, dataset_kind: str) -> dict | None:
        with SessionLocal() as session:
            record = session.execute(
                select(ImportBatchRecord)
                .where(ImportBatchRecord.dataset_kind == dataset_kind)
