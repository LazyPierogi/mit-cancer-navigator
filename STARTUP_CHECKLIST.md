...
## 1) Base Installation
- [x] Add universal rules from `.ai-playbook/distilled/universal/`.
- [ ] Select and add project-specific packs from `.ai-playbook/distilled/packs/`.
- [x] Create `AGENTS.md` and `PROJECT_MEMORY.md` from templates.
- [x] Run `SETUP_PROJECT_FIT_REVIEW.playbook.md` against real product docs to validate Skills + MCP fit.
- [x] Keep MCP source-of-truth in `mcp.json`. Current decision on 2026-02-28: remain in `skip` mode until the implementation stack requires runnable MCP servers; do not run `python3 scripts/update_windsurf_mcp.py` yet.
